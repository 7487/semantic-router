...
Repository Make targets derive the full commit from a clean checkout. For a
dirty checkout they build a canonical `sha256:` digest over every tracked and
non-ignored source path, file mode, object type, and byte. This keeps local
Evaluation available without weakening source identity; ignored benchmark data,
credentials, reports, caches, and other task-local artifacts are excluded.
