...
func newLedgerContractBroker(t *testing.T) (*workerHTTPBroker, *atomic.Int64) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/sealed-agent-tasks":
			...
			_, _ = writer.Write([]byte(`{"attempts":[{"attempt_id":"attempt-1"}]}`))
		case "/sealed-fault-recovery":
			...
			_, _ = writer.Write([]byte(`{"recoveries":[{"case_id":"case-1","attempt_id":"attempt-1"}]}`))
		case "/sealed-hard-policy":
			...
			_, _ = writer.Write([]byte(`{"assignments":[{"case_id":"case-1","attempt_id":"attempt-1"}]}`))
		case "/sealed-production-experiment":
			...
			_, _ = writer.Write([]byte(`{"observations":[{"case_id":"case-2","attempt_id":"attempt-2"}]}`))
		default:
...
func TestWorkerHTTPBrokerLedgerEndpointsAreExactServerOwnedContracts(t *testing.T) {
	...
	requests := []workerBrokerRequest{
		...
		{ID: 2, Operation: workerBrokerFaultRecoveryLedger, TrackID: "agentic", CaseID: "case-1", AttemptID: "attempt-1", Payload: json.RawMessage("null"), TimeoutMS: 2_000},
		{ID: 3, Operation: workerBrokerHardPolicyLedger, TrackID: "safety", CaseID: "case-1", AttemptID: "attempt-1", Payload: json.RawMessage("null"), TimeoutMS: 2_000},
		{ID: 4, Operation: workerBrokerProductionExperimentLedger, TrackID: "preference", CaseID: "case-2", AttemptID: "attempt-2", Payload: json.RawMessage("null"), TimeoutMS: 2_000},
	}
	...
	if broker.entries[1].responsePayload["attempts"] == nil || broker.entries[2].responsePayload["recoveries"] == nil || broker.entries[3].responsePayload["assignments"] == nil || broker.entries[4].responsePayload["observations"] == nil {
...
func newTypedChatBroker(t *testing.T, chatCalls *atomic.Int64) *workerHTTPBroker {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/v1/chat/completions":
			...
			writer.Header().Set("x-vsr-selected-algorithm", "weighted")
	...
	t.Cleanup(server.Close)
	broker := newWorkerHTTPBroker(
		RunManifest{SampleLimit: 4, Concurrency: 2, SuiteIDs: []string{"suite-a", "suite-b"}, TrackIDs: []TrackID{"model_pool", "joint"}, Target: ManifestTarget{
...
func TestWorkerHTTPBrokerPublishesOnlyBoundedTypedChatRequests(t *testing.T) {
	...
	if routedEntry.RequestedModel == nil || *routedEntry.RequestedModel != "virtual-entrypoint" ||
		...
		routedEntry.Algorithm == nil || *routedEntry.Algorithm != "weighted" ||
	...
	if routedEntry.SelectionStatus == nil || *routedEntry.SelectionStatus != "selected" ||
		routedEntry.SelectionMethod == nil || *routedEntry.SelectionMethod != "weighted" {
