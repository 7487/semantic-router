...
func TestControlledPairAggregatePublishesOneStrictPendingPair(t *testing.T) {
	...
	created, err := service.store.createControlledPairBundlesAs(actor, pair, baselineManifest, candidateManifest)
	if err != nil {
		t.Fatalf("create controlled pair aggregate: %v", err)
	}
...
func TestControlledPairIndexBatchNeverProjectsHalfPair(t *testing.T) {
	...
	service.store.controlledPairFault = func(point string) error {
...
func TestControlledPairAggregateEnforcesActorQuotaDestinationAndIdempotency(t *testing.T) {
	...
	t.Run("aggregate lifecycle ownership and retention", func(t *testing.T) {
		...
		hold := true
		if _, err := service.UpdateRunLifecycle(owner, pair.BaselineRunID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); err != nil {
		...
		if _, err := service.UpdateRunLifecycle(owner, pair.CandidateRunID, UpdateRunLifecycleRequest{RetentionClass: &protected}); err != nil {
