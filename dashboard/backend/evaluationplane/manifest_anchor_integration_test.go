...
func TestRealSealedRunKeepsSemanticAndArtifactManifestDigestsDistinct(t *testing.T) {
	...
	if sealReportErr := service.validateAndAnchorReport(run.ID); sealReportErr != nil {
	...
	if _, verifyReportErr := service.ReportJSON(run.ID); verifyReportErr != nil {
...
func TestCompareRejectsManifestArtifactTamperAfterBothRunsWereSealed(t *testing.T) {
	...
	if recovered, err := service.GetRun(baseline.ID); err != nil || recovered.Status != StatusCompleted {
	...
	if recovered, err := service.GetRun(candidate.ID); err != nil || recovered.Status != StatusCompleted {
		t.Fatalf("recover production-sealed candidate: run=%+v err=%v", recovered, err)
	}
	if _, err := service.Compare(baseline.ID, candidate.ID); err != nil {
	...
	if _, err := service.Compare(baseline.ID, candidate.ID); !errors.Is(err, ErrInvalid) {
