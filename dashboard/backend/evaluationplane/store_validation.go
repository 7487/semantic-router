...
func validateStoredRun(bundleID string, run Run) error {
	...
	if run.Mode == ModeLive || runUsesMoMCohortReplay(run) {
		if run.Mixture == nil || !targetIDMatchesMixture(run.TargetID, run.Mixture.ID) {
	...
	} else if run.Mixture != nil {
		return fmt.Errorf("replay status cannot contain a live mixture")
	}
...
func runUsesMoMCohortReplay(run Run) bool {
	return run.Mode == ModeReplay && len(run.SuiteIDs) == 1 && run.SuiteIDs[0] == "live-mom-core"
}
