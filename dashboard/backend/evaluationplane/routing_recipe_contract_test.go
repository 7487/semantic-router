...
func TestRoutingRecipeMetricSpecificationsAreConcreteAndAnchored(t *testing.T) {
	plan := testRoutingRecipePlan()
	plan.Signals[0].ID = "classifier:alpha:beta"
	plan.Projections[0].ID = "projection:alpha.beta"
	var err error
	...
	if err != nil {
		t.Fatalf("plan routing recipe metric specifications: %v", err)
	}
	if len(specifications) != 35 {
		t.Fatalf("metric specification count = %d, want 35", len(specifications))
	}
	found := false
	for _, specification := range specifications {
		if !IsRoutingRecipeMetricID(specification.ID) || !IsPlannedRoutingRecipeMetricID(plan, specification.ID) || specification.Missingness != "fail_closed" || specification.ClusterUnit != "case" || specification.EstimatorVersion != "v1" {
			t.Fatalf("invalid reducer-owned metric specification: %+v", specification)
		}
		if specification.ID == "routing_recipe.e2.feasible_oracle_recall_at_2" && specification.EstimatorID == "routing-recipe-e2-server-posthoc" {
			found = true
		}
	}
	if !found || IsRoutingRecipeMetricID("routing_recipe.e2.feasible_oracle_recall_at_0") || IsRoutingRecipeMetricID("routing_recipe.e2.projection.x.injected") || !IsRoutingRecipeMetricID("routing_recipe.e1.signal.626f677573.present_rate") || IsPlannedRoutingRecipeMetricID(plan, "routing_recipe.e1.signal.626f677573.present_rate") || IsRoutingRecipeMetricID("routing_recipe.e1.signal.7369676e616c2e616c7068612e62657461.present_rate.injected") {
		t.Fatalf("anchored dynamic metric grammar is not strict")
	}
}
