...
// PlanRunnableGradeableLiveMethods is the v2 catalog/planner projection.  It
// does not surface a method for a selected track unless the declaration has
// both complete live inputs and a live grader.
func PlanRunnableGradeableLiveMethods(methods []EvaluationMethodPlugin, selectedTracks []TrackID) ([]EvaluationMethodPlugin, error) {
	if len(selectedTracks) == 0 {
		return nil, fmt.Errorf("v2 live method planning requires selected tracks")
	}
	requested := make(map[TrackID]struct{}, len(selectedTracks))
	for _, track := range selectedTracks {
		if !validMethodTrackID(track) {
			return nil, fmt.Errorf("selected v2 method track is invalid")
		}
		requested[track] = struct{}{}
	}
	result := make([]EvaluationMethodPlugin, 0, len(methods))
	covered := make(map[TrackID]struct{}, len(requested))
	for _, method := range methods {
		if err := ValidateEvaluationMethodPlugin(method); err != nil {
			return nil, err
		}
		...
		for _, track := range method.LiveTracks {
			if _, selected := requested[track]; selected {
				matches = true
				covered[track] = struct{}{}
			}
		}
		if matches {
	...
	if len(covered) != len(requested) {
		return nil, fmt.Errorf("no runnable and gradeable v2 method covers every selected track")
	}
	return result, nil
}

// CaseArmObservation is deliberately generic so every method reducer gets the
// same duplicate case×arm protection instead of reimplementing it ad hoc.
type CaseArmObservation struct {
	CaseID string    `json:"case_id"`
	Action ActionRef `json:"action"`
	Value  float64   `json:"value"`
}

func ReduceCaseArmObservations(observations []CaseArmObservation) (map[string]map[string]float64, error) {
	if len(observations) == 0 {
		return nil, fmt.Errorf("case×action reducer requires observations")
	}
	reduced := make(map[string]map[string]float64)
	for _, observation := range observations {
		if !validMethodID(observation.CaseID) || validateActionRef(observation.Action) != nil ||
			math.IsNaN(observation.Value) || math.IsInf(observation.Value, 0) || observation.Value < 0 || observation.Value > 1 {
			return nil, fmt.Errorf("case×action observation is invalid")
		}
		byAction := reduced[observation.CaseID]
		if byAction == nil {
			byAction = make(map[string]float64)
			reduced[observation.CaseID] = byAction
		}
		if _, duplicate := byAction[observation.Action.ID]; duplicate {
			return nil, fmt.Errorf("duplicate case×action observation: %s×%s", observation.CaseID, observation.Action.ID)
		}
		byAction[observation.Action.ID] = observation.Value
	}
	return reduced, nil
}
