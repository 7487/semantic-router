...
func TestControlledPairPublicationCrashRecoveryHasNoHalfVisiblePair(t *testing.T) {
	for _, point := range []string{
		...
		t.Run(point, func(t *testing.T) {
			...
			reopened, reopenErr := NewStore(root)
			if reopenErr != nil {
				t.Fatalf("recover publication fault %s: %v", point, reopenErr)
			}
...
func TestControlledPairStartCrashRecoveryNeverExposesRunningPendingSplit(t *testing.T) {
	for _, point := range []string{
		"start_intent", "baseline_running", "candidate_running",
		"baseline_start_event", "candidate_start_event", "start_committed",
	} {
		t.Run(point, func(t *testing.T) {
			service, root := newControlledPairStoreTestService(t)
			pair, baselineManifest, candidateManifest := pendingControlledPairAggregate(t, service, SystemActor())
			...
			service.store.lifecycle.mu.Unlock()
			if startErr == nil {
				t.Fatalf("fault point %s did not interrupt start", point)
			}
			...
			reopened, reopenErr := NewStore(root)
			if reopenErr != nil {
				t.Fatalf("recover start fault %s: %v", point, reopenErr)
			}
	...
	for _, state := range []string{
		...
		t.Run(state, func(t *testing.T) {
			...
			case controlledPairStateDeleting:
				service.store.controlledPairFault = failControlledPairOnce("delete_intent")
				...
				service.store.controlledPairFault = nil
...
func TestControlledPairSubprocessCrashHelper(t *testing.T) {
	...
	store, openErr := NewStore(os.Getenv("VLLM_SR_PAIR_CRASH_ROOT"))
	if openErr != nil {
	...
	if mode := os.Getenv("VLLM_SR_PAIR_AUDIT_CRASH"); mode != "" {
		...
		if mode == "linked" {
			...
			record := lifecycleAuditRecord{
				...
				Timestamp: now, Action: "create", Decision: "allowed", ReasonCode: "system",
	...
	store.controlledPairFault = func(point string) error {
		if point == "baseline_running" {
			os.Exit(93)
		}
		return nil
	}
...
func TestControlledPairStartingEventReaderStopsAtVisibleBoundary(t *testing.T) {
	...
	service.store.controlledPairFault = failControlledPairOnce("baseline_running")
	service.store.lifecycle.mu.Lock()
	_, startErr := service.store.startControlledPairAs(SystemActor(), pair.PairID)
	service.store.lifecycle.mu.Unlock()
	if startErr == nil {
		t.Fatal("controlled pair start fault did not interrupt transition")
	}
...
func TestControlledPairAggregateCommitsTerminalStateAfterBothMembers(t *testing.T) {
	...
	if err := service.store.UpdateRun(candidate); err != nil {
...
func TestControlledPairRecoveryRejectsMembershipRoleSwap(t *testing.T) {
	...
	if _, err := NewStore(root); !errors.Is(err, ErrConflict) {
...
func TestControlledPairRecoveryRejectsStartingAndCancellingMutation(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "starting", faultPoint: "baseline_running",
		...
		{
			name: "cancelling", faultPoint: "baseline_cancelled",
			prepare: func(service *Service, pair controlledPairManifest) error {
				if _, err := service.store.startControlledPairAs(SystemActor(), pair.PairID); err != nil {
					return err
				}
				returnErr := error(nil)
				service.store.controlledPairFault = failControlledPairOnce("baseline_cancelled")
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.name == "starting" {
				service.store.controlledPairFault = failControlledPairOnce(test.faultPoint)
			}
...
func TestControlledPairCancelAndDeleteCrashRecoveryConverges(t *testing.T) {
	for _, point := range []string{"cancel_intent", "baseline_cancelled", "candidate_cancelled", "cancel_committed"} {
		t.Run(point, func(t *testing.T) {
			service, root := newControlledPairStoreTestService(t)
			pair, baselineManifest, candidateManifest := pendingControlledPairAggregate(t, service, SystemActor())
			if _, err := service.store.createControlledPairBundlesAs(SystemActor(), pair, baselineManifest, candidateManifest); err != nil {
			...
			if err != nil || !start.LaunchOwner {
				t.Fatalf("start pair: result=%+v err=%v", start, err)
			}
			service.store.controlledPairFault = failControlledPairOnce(point)
			service.store.lifecycle.mu.Lock()
			...
			service.store.lifecycle.mu.Unlock()
			if cancelErr == nil {
				t.Fatalf("fault point %s did not interrupt cancellation", point)
			}
			reopened, err := NewStore(root)
			if err != nil {
				t.Fatalf("recover cancellation at %s: %v", point, err)
			}
	...
	for _, point := range []string{"delete_intent", "delete_hidden", "baseline_deleted", "candidate_deleted", "delete_committed"} {
		t.Run(point, func(t *testing.T) {
			service, root := newControlledPairStoreTestService(t)
			pair, baselineManifest, candidateManifest := pendingControlledPairAggregate(t, service, SystemActor())
			if _, err := service.store.createControlledPairBundlesAs(SystemActor(), pair, baselineManifest, candidateManifest); err != nil {
			...
			service.store.lifecycle.mu.Unlock()
			if deleteErr == nil {
				t.Fatalf("fault point %s did not interrupt deletion", point)
			}
			reopened, reopenErr := NewStore(root)
			if reopenErr != nil {
				t.Fatalf("recover deletion at %s: %v", point, reopenErr)
			}
