...
// RoutingRecipeMetricSpecification is reducer-owned metadata for the public
// provenance registry. Callers must obtain the concrete IDs from the frozen
// plan rather than inventing a dynamic metric name at report construction.
type RoutingRecipeMetricSpecification struct {
	ID               string `json:"id"`
