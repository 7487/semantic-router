...
func TestLifecycleOwnerAuthorizationAndHashChainedAudit(t *testing.T) {
	...
	if _, err := service.UpdateRunLifecycle(other, run.ID, UpdateRunLifecycleRequest{
	...
	hold = true
	if _, err := service.UpdateRunLifecycle(owner, run.ID, UpdateRunLifecycleRequest{
		RetentionClass: &protected, EvidenceHold: &hold,
	}); err != nil {
	...
	if _, err := NewStore(service.store.root); err != nil {
...
func TestHeldProtectedAndReferencedRunsCannotBeCollected(t *testing.T) {
	...
	hold := true
	if _, holdErr := service.UpdateRunLifecycle(owner, baseline.ID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); holdErr != nil {
	...
	hold = false
	protected := RetentionProtected
	if _, protectErr := service.UpdateRunLifecycle(owner, baseline.ID, UpdateRunLifecycleRequest{
	...
	standard := RetentionStandard
	if _, restoreRetentionErr := service.UpdateRunLifecycle(owner, baseline.ID, UpdateRunLifecycleRequest{RetentionClass: &standard}); restoreRetentionErr != nil {
	...
	hold = true
	if _, holdCandidateErr := service.UpdateRunLifecycle(owner, candidate.ID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); holdCandidateErr != nil {
	...
	hold = false
	if _, releaseCandidateErr := service.UpdateRunLifecycle(owner, candidate.ID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); releaseCandidateErr != nil {
	...
	if _, err := service.GetRun(baseline.ID); err != nil {
...
func TestCancelledRunParticipatesInRetentionCollection(t *testing.T) {
	...
	if persistCancelledErr := service.store.UpdateRun(run); persistCancelledErr != nil {
...
func TestLifecyclePublicationFaultsAndConcurrentMutationFailClosed(t *testing.T) {
	...
	hold := true
	if _, publicationErr := service.UpdateRunLifecycle(owner, run.ID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); publicationErr == nil {
	...
	for index := 0; index < 40; index++ {
		...
		go func(index int) {
			...
			_, _ = service.UpdateRunLifecycle(owner, run.ID, UpdateRunLifecycleRequest{
...
func TestLifecycleRestartQuarantinesRunPolicyAndRejectsAuditCorruption(t *testing.T) {
	t.Run("run lifecycle quarantine", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("restart quarantined lifecycle: %v", err)
		}
		ledger, err := restarted.listRunLedger(RunListQuery{Limit: 10})
		if err != nil || ledger.LedgerComplete || ledger.WarningCount != 1 || len(ledger.Runs) != 0 {
			t.Fatalf("lifecycle corruption was not quarantined: ledger=%+v err=%v", ledger, err)
		}
	...
	t.Run("audit fails startup", func(t *testing.T) {
		service, root := newTestService(t, &controlledProcess{}, 1)
		if _, err := service.CreateRun(context.Background(), validCreateRequest()); err != nil {
			t.Fatalf("CreateRun: %v", err)
		}
