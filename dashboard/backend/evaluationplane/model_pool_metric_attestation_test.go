...
func TestCanonicalModelPoolMetricSpecificationIsExact(t *testing.T) {
	spec := CanonicalModelPoolMetricSpecification()
	if spec.AnalysisUnit != "planned_case_x_frozen_arm" || spec.DynamicIDPattern == "" || len(spec.ProjectionEstimatorSet) != 6 {
		t.Fatalf("unexpected metric specification: %#v", spec)
	}
	for _, id := range spec.StaticIDs {
