...
func (s *Store) deleteRunAuthorizedUnlocked(actor Actor, id string) error {
	...
	if _, auditErr := s.appendLifecycleAuditUnlocked(
		actor, "delete", "allowed", lifecycleAuthorizationReason(actor, lifecycle), id, lifecycle.OwnerPrincipalDigest,
	); auditErr != nil {
		return auditErr
	}
	if removeErr := os.RemoveAll(runDir); removeErr != nil {
		return fmt.Errorf("delete evaluation run: %w", removeErr)
	}
	s.runIndex.remove(id)
	if syncErr := syncEvaluationDirectory(s.runsRoot, "evaluation runs"); syncErr != nil {
		return syncErr
	}
	removedAttestation, attestationErr := s.removeExecutionAttestationIfPresent(id)
	if attestationErr != nil {
		return attestationErr
	}
	if removedAttestation {
		if syncErr := syncEvaluationDirectory(s.attestationRoot, "evaluation execution attestation deletion"); syncErr != nil {
			return syncErr
		}
	}
	if _, auditErr := s.appendLifecycleAuditUnlocked(
		actor, "gc", "allowed", "delete_cascade", "", "",
	); auditErr != nil {
		return auditErr
	}
	if sweepErr := s.sweepUnreferencedCASUnlocked(); sweepErr != nil {
		log.Printf("evaluationplane: CAS collection deferred after run deletion: %v", sweepErr)
	}
	return nil
}
...
func (s *Store) recoverCASGarbage() {
	...
	if _, auditErr := s.appendLifecycleAuditUnlocked(
		SystemActor(), "gc", "allowed", "startup_recovery", "", "",
	); auditErr != nil {
...
func (s *Store) markRunCASReferences(runID string, references map[string]bool) error {
	...
	for _, name := range workerRunArtifactNames {
		if !present[name] || name == "events.jsonl" {
			continue
		}
