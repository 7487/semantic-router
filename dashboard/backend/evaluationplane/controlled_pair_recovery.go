...
func (s *Store) recoverControlledPairTransactionsUnlocked() error {
	...
	for _, entry := range entries {
		...
		hasManifest, hasTombstone, err := cleanupControlledPairAtomicTemps(dir)
		if err != nil {
			return err
		}
		if !hasManifest {
	...
	return syncEvaluationDirectory(s.controlledPairRoot, "controlled pair recovery")
}
...
func (s *Store) recoverControlledPairPublication(pair controlledPairManifest) error {
	...
	if err := syncEvaluationDirectory(s.runsRoot, "controlled pair publication recovery"); err != nil {
		return err
	}
	pending := pair
...
func (s *Store) rollbackControlledPairPublication(pair controlledPairManifest) error {
	...
	if err := syncEvaluationDirectory(s.runsRoot, "controlled pair publication rollback"); err != nil {
		return err
	}
	return syncEvaluationDirectory(s.controlledPairRoot, "controlled pair transaction rollback")
}
