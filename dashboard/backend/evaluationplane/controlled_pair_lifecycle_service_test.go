...
func TestControlledPairAuthoritativeReadUsesAggregateStateAndOwnership(t *testing.T) {
	...
	if err := service.store.UpdateRun(half); err != nil {
	...
	if err := service.store.UpdateRun(sealing); err != nil {
...
func TestControlledPairCapabilitiesAndDeleteSeeWorkersOwnedByAnotherService(t *testing.T) {
	...
	execution, operationErr := service.CreateControlledPairExecution(context.Background(), request)
	if operationErr != nil {
	...
	for _, run := range []Run{execution.BaselineRun, execution.CandidateRun} {
		...
		if err := service.store.UpdateRun(terminal); err != nil {
			t.Fatal(err)
		}
	}
	...
	second, operationErr := NewService(Options{
...
func TestControlledPairCancelFromSecondServiceStopsOwnerWorkers(t *testing.T) {
	...
	if _, err := owner.CreateControlledPairExecution(context.Background(), request); err != nil {
		t.Fatal(err)
	}
	for range 2 {
		select {
...
func TestControlledPairReadCapabilitiesPreflightActiveAndLifecyclePolicy(t *testing.T) {
	...
	hold := true
	if _, err := service.UpdateRunLifecycle(actor, pair.CandidateRunID, UpdateRunLifecycleRequest{EvidenceHold: &hold}); err != nil {
	...
	if _, err := service.UpdateRunLifecycle(actor, pair.CandidateRunID, UpdateRunLifecycleRequest{
	...
	if _, err := service.UpdateRunLifecycle(actor, pair.CandidateRunID, UpdateRunLifecycleRequest{RetentionClass: &standard}); err != nil {
	...
	campaignDir := filepath.Join(service.store.Root(), "campaigns", campaign.ID)
	if err := os.Mkdir(campaignDir, 0o700); err != nil {
		t.Fatalf("create campaign reference fixture: %v", err)
	}
	if err := writeJSONAtomic(filepath.Join(campaignDir, campaignFileName), campaign); err != nil {
		t.Fatalf("write campaign reference fixture: %v", err)
	}
	execution, operationErr = service.GetControlledPairExecutionAs(actor, pair.PairID)
	if operationErr != nil || execution.Capabilities.CanDelete {
...
func TestControlledPairDeleteClosesBothMemberSubscriptions(t *testing.T) {
	...
	baselineEvents, _, err := service.Subscribe(pair.BaselineRunID)
	if err != nil {
		t.Fatalf("subscribe baseline: %v", err)
	}
	candidateEvents, _, err := service.Subscribe(pair.CandidateRunID)
	if err != nil {
	...
	if service.subscriberCount != 0 || len(service.subscribers) != 0 {
		t.Fatalf("pair subscribers leaked: count=%d maps=%d", service.subscriberCount, len(service.subscribers))
	}
}
