...
func pendingControlledPairAggregate(
	...
	t.Helper()
	baselineSource := createStoreControlledPairSource(t, service, actor)
	candidateSource := createStoreControlledPairSource(t, service, actor)
...
func createStoreControlledPairSource(t *testing.T, service *Service, actor Actor) controlledPairSource {
	t.Helper()
	deployments, loadErr := LoadEvaluationDeploymentRegistry(service.deploymentsDir, "")
	if loadErr != nil || len(deployments) == 0 {
		t.Fatalf("load controlled pair source deployment: count=%d err=%v", len(deployments), loadErr)
	}
	run, createErr := service.CreateRunAs(context.Background(), actor, CreateRunRequest{
		...
		Mode: ModeLive, TargetID: deployments[0].TargetID, ChangeProfile: "recipe",
...
func failControlledPairOnce(point string) func(string) error {
	fired := false
	return func(got string) error {
		if got == point && !fired {
			fired = true
			return errors.New("injected controlled pair crash")
		}
		return nil
	}
}
