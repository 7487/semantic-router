...
func TestRoutingRecipeDecisionMutationInvalidatesBrokerReceiptAndAttestation(t *testing.T) {
	...
	selectionMethod, decisionName := "weighted", "quality"
...
func TestBrokerCaseRequestBindingRejectsPromptSubstitution(t *testing.T) {
	...
	requestDigest, digestErr := brokerRequestDigestForMessages(workerBrokerRouterEvaluate, model, forgedDigest)
	if digestErr != nil {
	...
	entry.RequestDigest, digestErr = brokerRequestDigestForMessages(workerBrokerRouterEvaluate, model, plannedDigest)
	if digestErr != nil {
