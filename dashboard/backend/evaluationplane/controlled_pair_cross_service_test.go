...
func TestControlledPairConcurrentCreateIsSingleFlightAcrossServices(t *testing.T) {
	...
	second, err := NewService(Options{
		DataDir: first.store.Root(), PythonPath: "python3", ConfigPath: first.configPath,
		DeploymentsDir: first.deploymentsDir, CodeRevision: testSourceRevision,
	...
	go func() {
		execution, createErr := first.CreateControlledPairExecution(context.Background(), request)
	...
	go func() {
		close(secondStarted)
		execution, createErr := second.CreateControlledPairExecution(context.Background(), request)
...
func assertControlledPairSingleFlightResult(
	...
	if _, err := second.CreateControlledPairExecution(context.Background(), conflict); !errors.Is(err, ErrConflict) {
...
func TestControlledPairOwnerCancellationReleasesLifecycleAndSharedFlight(t *testing.T) {
	...
	go func() {
		execution, err := first.CreateControlledPairExecution(ctx, request)
...
func TestControlledPairPreCancelledOwnerDoesNotPublishOrReserve(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := service.CreateControlledPairExecution(ctx, request); !errors.Is(err, context.Canceled) {
	...
	service.mu.Unlock()
	if calls := process.calls.Load(); calls != 0 || len(service.semaphore) != 0 || prelaunches != 0 {
		t.Fatalf("pre-cancelled owner leaked work: calls=%d slots=%d prelaunches=%d", calls, len(service.semaphore), prelaunches)
	}
}
...
func TestControlledPairPreCancelledFollowerDoesNotJoinSharedFlight(t *testing.T) {
	...
	go func() {
		execution, err := owner.CreateControlledPairExecution(context.Background(), request)
	...
	follower, err := NewService(Options{
		DataDir: owner.store.Root(), PythonPath: "python3", ConfigPath: owner.configPath,
		DeploymentsDir: owner.deploymentsDir, CodeRevision: testSourceRevision,
		MaxConcurrent: 2, Process: process,
	})
	if err != nil {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := follower.CreateControlledPairExecution(ctx, request); !errors.Is(err, context.Canceled) {
		t.Fatalf("pre-cancelled follower error=%v, want context.Canceled", err)
	}
	follower.mu.Lock()
	prelaunches := follower.prelaunchCount
	follower.mu.Unlock()
	if len(follower.semaphore) != 0 || prelaunches != 0 {
		t.Fatalf("pre-cancelled follower leaked work: slots=%d prelaunches=%d", len(follower.semaphore), prelaunches)
	}
	select {
...
func TestControlledPairCloseCancelsAndDrainsPrelaunch(t *testing.T) {
	...
	result := make(chan controlledPairCreateResult, 1)
	go func() {
		execution, err := first.CreateControlledPairExecution(context.Background(), request)
		result <- controlledPairCreateResult{execution: execution, err: err}
	}()
	select {
	case <-process.freezeEntered:
...
func TestControlledPairFollowerCloseCancelsSharedFlightWait(t *testing.T) {
	...
	go func() {
		execution, createErr := follower.CreateControlledPairExecution(context.Background(), request)
