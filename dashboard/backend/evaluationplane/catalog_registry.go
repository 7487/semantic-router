...
func mixtureTargetDefinition(
	...
	healthy := false
	labels := map[string]string{
		"capabilities": "mixture-bound", "credentials": "environment-only", "model_arms": "server-owned",
	}
	if options.RouterAuthRequired && options.RouterAPIKey == nil {
