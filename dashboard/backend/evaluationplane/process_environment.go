...
func (p *CommandProcess) workerEnvironment(manifestPath, sandboxRoot string) ([]string, error) {
	manifest, _, err := readRunManifestStrict(manifestPath)
	if err != nil {
		return nil, fmt.Errorf("read staged evaluation manifest: %w", err)
	}
	if _, err := p.brokerCredentials(manifest); err != nil {
		return nil, err
	}
	return isolatedWorkerEnvironment(sandboxRoot), nil
}
