...
func (s *Store) cancelControlledPairAs(actor Actor, pairID string) (controlledPairManifest, error) {
	...
	for _, runID := range []string{pair.BaselineRunID, pair.CandidateRunID} {
		if _, err := s.appendLifecycleAuditUnlocked(
			actor, "cancel", "allowed", lifecycleAuthorizationReasonForPair(actor, pair),
	...
	if err := s.controlledPairFail("cancel_intent"); err != nil {
		return controlledPairManifest{}, err
	}
...
func (s *Store) finishControlledPairCancellation(pair controlledPairManifest) error {
	...
	for index, snapshot := range []Run{pair.BaselineRun, pair.CandidateRun} {
		...
		case run.Status == StatusRunning:
			...
			if err := s.statusPersistence.Write(filepath.Join(s.runsRoot, run.ID, runFileName), run); err != nil {
				return err
			}
		default:
			return fmt.Errorf("%w: controlled pair member cannot be cancelled from %s", ErrConflict, run.Status)
		}
		point := "baseline_cancelled"
		if index == 1 {
			point = "candidate_cancelled"
		}
		if err := s.controlledPairFail(point); err != nil {
			return err
		}
	...
	if err := s.controlledPairFail("cancel_committed"); err != nil {
		return err
	}
...
func (s *Store) deleteControlledPairAs(actor Actor, pairID string) error {
	...
	for _, runID := range []string{pair.BaselineRunID, pair.CandidateRunID} {
		if _, err := s.appendLifecycleAuditUnlocked(
			actor, "delete", "allowed", lifecycleAuthorizationReasonForPair(actor, pair), runID, pair.OwnerPrincipalDigest,
		); err != nil {
			return err
		}
	}
	if _, err := s.appendLifecycleAuditUnlocked(SystemActor(), "gc", "allowed", "delete_cascade", "", ""); err != nil {
		return err
	}
	...
	if err := s.controlledPairFail("delete_intent"); err != nil {
		return err
	}
	s.runIndex.removeBatch(pair.BaselineRunID, pair.CandidateRunID)
	if err := s.controlledPairFail("delete_hidden"); err != nil {
		return err
	}
	return s.finishControlledPairDeletion(pair)
}
...
func (s *Store) finishControlledPairDeletion(pair controlledPairManifest) error {
	for index, runID := range []string{pair.BaselineRunID, pair.CandidateRunID} {
		...
		if _, statErr := os.Lstat(runDir); statErr == nil {
			...
			if membershipErr := s.requireControlledPairMembership(runDir, pair.PairID, runID, role); membershipErr != nil {
		...
		point := "baseline_deleted"
		if index == 1 {
			point = "candidate_deleted"
		}
		if err := s.controlledPairFail(point); err != nil {
			return err
		}
	}
	if err := syncEvaluationDirectory(s.runsRoot, "controlled pair deletion"); err != nil {
		return err
	}
	...
	if err := s.controlledPairFail("delete_committed"); err != nil {
		return err
	}
	s.runIndex.removeBatch(pair.BaselineRunID, pair.CandidateRunID)
