...
// ModelPoolMetricProjection documents the fixed planned cohort and estimator a
// future provenance sealer must preserve when it publishes a reduced value.
type ModelPoolMetricProjection struct {
	MetricIDs       []string
	AnalysisUnit    string
	EstimatorFamily string
}

// ModelPoolMetricSpecification is the single reducer-owned public contract for
// model-pool metrics. Static IDs are exact; only the anchored dynamic grammar
// may produce an arm-specific ID. This is deliberately separate from report
// schema integration while that integration is being changed elsewhere.
type ModelPoolMetricSpecification struct {
	StaticIDs              []string
	DynamicIDPattern       string
	AnalysisUnit           string
	ProjectionEstimatorSet []ModelPoolMetricProjection
}
...
// CanonicalModelPoolMetricSpecification lets the provenance/report owner use
// the reducer's single metric contract without duplicating identifiers or
// estimator claims in another layer.
func CanonicalModelPoolMetricSpecification() ModelPoolMetricSpecification {
	return ModelPoolMetricSpecification{
		StaticIDs:        append([]string(nil), modelPoolStaticMetricIDs...),
		DynamicIDPattern: modelPoolDynamicMetricIDPattern.String(),
		AnalysisUnit:     "planned_case_x_frozen_arm",
		ProjectionEstimatorSet: []ModelPoolMetricProjection{
			{MetricIDs: []string{"model_pool.arm_count", "model_pool.quality_shared_support_cases", "model_pool.quality_shared_support_fraction", "model_pool.quality_cost_shared_support_cases", "model_pool.quality_cost_shared_support_fraction"}, AnalysisUnit: "frozen_arm_set_or_planned_case_x_frozen_arm", EstimatorFamily: "frozen_set_count_or_dense_support_count"},
			{MetricIDs: []string{"model_pool.oracle_quality", "model_pool.best_single_quality", "model_pool.oracle_gain", "model_pool.unique_wins", "model_pool.unique_win_rate", "model_pool.quality_dominated_arm_count"}, AnalysisUnit: "planned_case_x_frozen_arm", EstimatorFamily: "dense_case_mean_or_count"},
			{MetricIDs: []string{"model_pool.arm.<portable_segment(arm_id)>.quality", "model_pool.arm.<portable_segment(arm_id)>.success_rate", "model_pool.arm.<portable_segment(arm_id)>.marginal_contribution"}, AnalysisUnit: "planned_case_x_frozen_arm", EstimatorFamily: "dense_arm_case_mean"},
			{MetricIDs: []string{"model_pool.all_arm_failure_rate", "model_pool.mean_pairwise_failure_jaccard", "model_pool.worst_arm_reliability"}, AnalysisUnit: "planned_case_x_frozen_arm", EstimatorFamily: "dense_case_failure_set"},
			{MetricIDs: []string{"model_pool.pareto_evaluable_arm_count", "model_pool.pareto_dominated_arm_count"}, AnalysisUnit: "planned_case_x_frozen_arm", EstimatorFamily: "mean_quality_runtime_cost_pareto"},
			{MetricIDs: []string{"model_pool.selection_entropy_bits", "model_pool.selection_arm_coverage"}, AnalysisUnit: "planned_case", EstimatorFamily: "joint_selection_distribution"},
		},
	}
}

// IsCanonicalModelPoolMetricID accepts exactly the static set or an
// arm-specific ID defined by CanonicalModelPoolMetricSpecification.
...
// ModelPoolMetricArmSegment produces the one-segment, reversible arm
// dimension used in a metric ID. Raw arm IDs permit dots, so they must never
// be interpolated directly into a dot-delimited metric path.
...
// ParseCanonicalModelPoolArmMetricID decodes an arm-specific model-pool
// metric without a split heuristic. It rejects non-canonical direct or
// base64url segments and identities that cannot be frozen arm IDs.
