...
func TestCampaignV2CatalogIsTheSlotSourceOfTruth(t *testing.T) {
	for _, profile := range builtinChangeProfiles() {
		...
		for index, slot := range profile.CampaignSlots {
			gateIndex := index + 2
			if slot.GateID != requiredGateIDs[gateIndex] || slot.Disposition != gateDispositionMatrix[profile.ID][gateIndex] {
				t.Fatalf("profile %s slot=%+v report matrix=%q", profile.ID, slot, gateDispositionMatrix[profile.ID][gateIndex])
			}
		}
	}
