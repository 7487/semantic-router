...
func (s *Store) AppendEvent(event Event) (Event, error) {
	...
	file, openErr := openBundleFile(filepath.Join(runDir, eventsFileName), os.O_WRONLY|os.O_APPEND)
	if openErr != nil {
		return Event{}, fmt.Errorf("open evaluation event log: %w", openErr)
	}
	...
	closeErr := file.Close()
	if writeErr != nil {
		return Event{}, fmt.Errorf("append evaluation event: %w", writeErr)
	}
	if closeErr != nil {
