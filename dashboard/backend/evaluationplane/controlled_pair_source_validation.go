...
func (s *Service) loadControlledPairSource(id string) (controlledPairSource, error) {
	...
	anchorBytes, err := readEvidenceBytes(
		filepath.Join(s.store.runsRoot, id, reportAnchorFileName), maxReportAnchorBytes,
	)
	if err != nil {
...
func (s *Service) readControlledPairSource(id string) (controlledPairSource, error) {
	if s.controlledPairSourceRead != nil {
		return s.controlledPairSourceRead(id)
	}
	return s.loadControlledPairSource(id)
}
