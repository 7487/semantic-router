...
func TestCommandProcessFixtureEndToEnd(t *testing.T) {
	...
	reportBytes, reportErr := service.ReportJSON(run.ID)
	if reportErr != nil {
...
func assertSafetyBundleSealRejected(t *testing.T, options safetyReportOptions, match string) {
	...
	err = service.validateAndAnchorReport(run.ID)
...
func writeSafetyEvidenceArtifacts(
	...
	for _, name := range []string{"report.md", "report.html"} {
		if err := os.WriteFile(filepath.Join(runDir, name), []byte("forged rendered report\n"), 0o600); err != nil {
			return nil, err
		}
	}
