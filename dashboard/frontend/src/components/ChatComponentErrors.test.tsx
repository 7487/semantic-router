...
describe('ChatComponentErrors', () => {
  ...
  it('anchors the error region to the top of the chat content', () => {
    const styles = readFileSync(new URL('./ChatComponent.module.css', import.meta.url), 'utf8')

    expect(styles).toMatch(/\.chatArea\s*{[^}]*position:\s*relative;/s)
    expect(styles).toMatch(
      /\.errorRegion\s*{[^}]*position:\s*relative;[^}]*flex:\s*0 0 auto;[^}]*width:\s*100%;/s,
    )
    expect(styles).toMatch(
      /\.chatAreaEmpty \.errorRegion\s*{[^}]*position:\s*absolute;[^}]*inset:\s*0 0 auto;/s,
    )
    expect(styles).toMatch(
      /\.error\s*{[^}]*box-sizing:\s*border-box;[^}]*max-width:\s*var\(--chat-rail-width\);/s,
    )
  })
})
