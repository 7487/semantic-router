...
export function releaseDecisionSummary(
  ...
  switch (verdict) {
    ...
    case 'waived':
      return 'Release can proceed with documented exceptions.'
    case 'not_applicable':
      return 'No release decision is required for this change.'
  }
}
...
export function releaseNextActions(
  ...
  const nextByVerdict: Record<typeof verdict, string> = {
    ...
    waived: 'Review the documented exceptions before proceeding with the release.',
    not_applicable: 'No release action is required for this change.',
  }
