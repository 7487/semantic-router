...
export function pairedCampaignCohortMismatches(
  ...
  if (baseline.target_id === candidate.target_id) mismatches.push('candidate Mixture')
  if (
    !baseline.mixture ||
    !candidate.mixture ||
    ...
    mismatches.push('Mixture version')
  }
  ...
  if (!sameOrdered(baseline.suite_ids, candidate.suite_ids)) mismatches.push('benchmark selection')
  if (!sameOrdered(baseline.track_ids, candidate.track_ids)) mismatches.push('evaluation areas')
  return mismatches
}

function fidelityCohortMismatches(reference: EvaluationRun, live: EvaluationRun): string[] {
  const mismatches: string[] = []
  if (reference.change_profile !== live.change_profile) mismatches.push('change being evaluated')
  if (reference.target_id !== live.target_id) mismatches.push('tested Mixture')
  if (reference.sample_limit !== live.sample_limit) mismatches.push('sample size')
  if (reference.seed !== live.seed) mismatches.push('repeatability setting')
  if (!sameOrdered(reference.suite_ids, live.suite_ids)) mismatches.push('benchmark selection')
  if (!sameOrdered(reference.track_ids, live.track_ids)) mismatches.push('evaluation areas')
  if (
    !reference.completed_at ||
    !live.started_at ||
    Date.parse(live.started_at) <= Date.parse(reference.completed_at)
  ) {
    mismatches.push('live reference selection')
  }
  return mismatches
}
...
function campaignRunMatchesSlot(
  run: EvaluationRun,
  ...
  for (const slot of profile.campaign_slots) {
    ...
    if (slot.binding_kind === 'controlled_pair' && ids.length === 2) {
      ...
      if (
        ...
        return 'Launch a fresh controlled comparison for the selected baseline and candidate.'
      }
    }
    if (slot.binding_kind === 'fidelity_pair' && ids.length === 2) {
      ...
      if (
        ...
        return 'Select a matching reference and fresh candidate run.'
      }
    }
  }
  return null
