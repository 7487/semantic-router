...
export function decodeEvaluationRun(payload: unknown, expectedID?: string): EvaluationRun {
  ...
  const capacityRequired = requiresCapacitySLO(
    ...
    payload.track_ids as EvaluationRun['track_ids'],
  )
  if (capacityRequired && payload.concurrency < 2) {
    throw new Error('Live capacity run requires concurrency of at least 2.')
  }
  if (
    capacityRequired &&
    (payload.capacity_slo === undefined || payload.capacity_load_protocol === undefined)
  ) {
  ...
  if (payload.capacity_slo !== undefined) {
    const capacitySLO = decodeEvaluationCapacitySLO(
      payload.capacity_slo,
      'Evaluation run Capacity SLO',
    )
    if (capacitySLO.required_concurrency > payload.concurrency) {
      throw new Error('Evaluation run Capacity SLO exceeds its run concurrency.')
    }
  }
  if (payload.capacity_load_protocol !== undefined) {
    decodeEvaluationCapacityLoadProtocol(
      payload.capacity_load_protocol,
      payload.concurrency as number,
  ...
  return payload as unknown as EvaluationRun
}
