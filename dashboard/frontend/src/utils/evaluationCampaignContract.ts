...
const CHANGE_PROFILES = new Set<string>(EVALUATION_CHANGE_PROFILE_IDS)
const EVIDENCE_LEVELS = new Set(['E0', 'E1', 'E2', 'E3', 'E4', 'E5'])
const GATE_DISPOSITIONS = new Set(['required', 'advisory', 'not_applicable', 'waived'])
const GATE_VERDICTS = new Set(['pass', 'fail', 'unavailable', 'waived', 'not_applicable'])
const DECISION_VERDICTS = new Set(['pass', 'fail', 'unavailable'])
...
function known(value: unknown, values: Set<string>): value is string {
...
function decodeGate(value: unknown, index: number): EvaluationCampaignGate {
  ...
  if (
    ...
    value.id !== gateID ||
    !text(value.name) ||
    !known(value.disposition, GATE_DISPOSITIONS) ||
    !known(value.verdict, GATE_VERDICTS) ||
  ...
  if (
    ...
    (value.disposition === 'waived' && value.verdict !== 'waived') ||
    ((value.disposition === 'required' || value.disposition === 'advisory') &&
      (value.verdict === 'not_applicable' || value.verdict === 'waived')) ||
...
function decodeDecision(
  ...
  if (
    ...
    !known(value.verdict, DECISION_VERDICTS) ||
    !text(value.summary) ||
    !Array.isArray(value.gates) ||
    value.gates.length !== GATE_IDS.length ||
    !Array.isArray(value.evidence) ||
    !texts(value.recommendations) ||
    !timestamp(value.created_at)
  ) {
    throw new Error('Evaluation campaign decision is incomplete.')
  }
...
export function decodeEvaluationCampaign(
  ...
  if (
    ...
    !known(payload.change_profile, CHANGE_PROFILES) ||
...
export function buildCreateEvaluationCampaignPayload(
  ...
  if (!CHANGE_PROFILES.has(request.change_profile)) {
    throw new Error('Campaign change profile must use the current evaluation contract.')
  }
  return {
