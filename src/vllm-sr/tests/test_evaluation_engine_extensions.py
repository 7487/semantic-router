...
from cli.evaluation.catalog import CatalogMethod, CatalogSuite, get_catalog
...
def test_run_evidence_is_weakest_selected_track_and_unavailable_is_e0() -> None:
    joint = ExecutionRecord(
        ...
        status="succeeded",
        evidence_kind="qualified-live.v1;level=E5",
        success=True,
    )
    ...
    executor_id = "qualified-provider.v1"
    assert track_evidence_level("live", executor_id, "joint", [joint]) == "E5"
    assert track_evidence_level("live", executor_id, "safety", [safety]) == "E0"
    assert (
        run_evidence_level(
            "live", executor_id, ("joint", "safety"), [joint, safety], None
        )
        == "E0"
    )
    assert run_evidence_level("live", executor_id, ("joint",), [joint], "E0") == "E0"
