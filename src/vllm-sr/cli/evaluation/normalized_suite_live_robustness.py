...
def _target_arm_id(
    selector: str | None, arms: tuple[EvaluationTargetArm, ...]
) -> str | None:
    if selector is None:
        return None
    matches = [arm.id for arm in arms if selector in {arm.id, arm.model}]
    ...
    *,
    ...
    for pair in pairs:
        ...
        if not _routing_pair_is_complete(source, target):
            return None
        expected_action = _target_arm_id(pair.expected_action_id, arms)
        ...
        (
            record.model_copy(
                update={
                    ...
                    "evidence_kind": DECLARED_SHIFT_LIVE_EVIDENCE_KIND,
                }
            )
...
def declared_shift_gate_is_complete(records: list[ExecutionRecord]) -> bool:
    ...
    return (
        ...
        and all(
            ...
            and row.evidence_kind == DECLARED_SHIFT_LIVE_EVIDENCE_KIND
            and row.broker_receipt is not None
            for row in records
