...
def _plugin(entry: MappingProxyType | dict[str, Any]) -> EvaluationMethodPlugin:
    if entry["adapter_id"] == "r2-router":
        return R2_COMPOUND_MODEL_BUDGET_PLUGIN
    return EvaluationMethodPlugin(
        ...
        analysis_plan=AnalysisPlan(
            id=f"{entry['adapter_id']}-case-cluster",
            analysis_unit="case_action",
            cluster_unit="case",
            slices=(SliceRef(id="all"),),
            curve_domain="not_applicable",
