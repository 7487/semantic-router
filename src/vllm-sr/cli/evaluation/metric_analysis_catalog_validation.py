...
EXPECTED_RETIRED_METRIC_COUNT = 11
...
def _validate_retired_metrics(
    document: Mapping[str, Any], static_ids: list[str]
) -> None:
    retired = document["retired_metric_ids"]
    if not isinstance(retired, list) or len(retired) != EXPECTED_RETIRED_METRIC_COUNT:
...
def validate_document(document: dict[str, Any]) -> None:
    ...
    static_ids = _validate_static_metrics(document, refs, compiled)
    _validate_retired_metrics(document, static_ids)
