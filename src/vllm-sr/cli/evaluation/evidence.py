    ...
    def _validate_live_robustness_binding(self) -> None:
        if (
            ...
            if (
                ...
                or self.evidence_kind != "declared-shift.server-live.v1;level=E4"
            ):
                raise ValueError(
    ...
    def _validate_agent_task_binding(self) -> None:
        if self.agent_task is not None:
            ...
            if (
                ...
                or not _float_equal(self.runtime_cost, method.runtime_cost_usd)
                or self.evaluation_cost is None
                or not _float_equal(self.evaluation_cost, method.evaluation_cost_usd)
                or self.grader != method.grader_id
                or self.evidence_kind != AGENT_TASK_EVIDENCE_KIND
                or not self.broker_receipt
            ):
                raise ValueError(
