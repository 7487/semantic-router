...
class TargetRegistry:
    ...
    def resolve(
        ...
        if (
            ...
            or executor.id not in contract.accepted_executors[manifest.mode]
    ...
    def _validate_target_shape(manifest: RunManifest, contract: TargetContract) -> None:
        ...
        if contract.execution_profile == "recorded-source":
            ...
            return
        if target.router_api_key is not None:
            raise ValueError(
                "brokered-runtime target does not support a Router evaluation credential"
            )
        if (
